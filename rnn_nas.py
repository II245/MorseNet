...
def main():
    ...
    progression_grid = {
        'rnn_size': [64],
        'lr': [0.005],
        'kernel_initializer': ['lecun_uniform', 'glorot_uniform', 'he_normal'] # ['he_normal', 'normal', 'uniform']
    }
    ...
    models = {
        ...
        'progression_v10': (lambda arch: get_progression_model_v10(RMSprop(lr=arch['lr']), arch), progression_grid),
        'progression_v11': (lambda arch: get_progression_model_v11(RMSprop(lr=arch['lr']), arch), progression_grid),
        'progression_v12': (lambda arch: get_progression_model_v12(RMSprop(lr=arch['lr']), arch), progression_grid),
        'progression_v13': (lambda arch: get_progression_model_v13(RMSprop(lr=arch['lr']), arch), progression_grid),
    }
    ...
    for model_name, (model_lambda, grid) in models.items():
        for arch in ParameterGrid(grid):
            for try_index in range(3):
                ...
                fit_history_0 = model.fit(X, l, validation_split=0.1, batch_size=1000, callbacks=[viz_cb], epochs=1)
                fit_history = model.fit(X, l, validation_split=0.1, batch_size=32, callbacks=[viz_cb], epochs=3)

                print(fit_history.history)

                best_loss = np.min(fit_history.history['loss'])
                best_val = np.min(fit_history.history['val_loss'])

                open('output/nas_results_2.txt', 'a+').write(f'{model_name}\t{try_index}\t{best_val}'
                                                    f'\t{best_loss}\t{arch}\t{fit_history.history}\n')
